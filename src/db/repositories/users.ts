...
import type { AccountMode, TradeMode, UserProfile, UserStep } from '../../types';
...
export async function getUser(chatId: number): Promise<UserProfile | null> {
  ...
  return {
    ...
    accountMode: (data.account_mode as AccountMode) ?? 'simulation',
  };
}
...
export async function updateAccountMode(chatId: number, mode: AccountMode): Promise<void> {
  const { error } = await supabase
    .from('users')
